...
async def register_participant(
    ...
    if existing:
        raise HTTPException(
            status_code=400,
            detail="Email or GitHub username already registered"
        )
...
async def submit_project(
    ...
    if not verify_github_repo(
        ...
        raise HTTPException(
            status_code=400,
            detail="Invalid repository or missing required files"
        )
...
async def get_statistics(db: Session = Depends(get_db)):
    ...
    total_submitted = db.query(Participant).filter(
        Participant.submission_url != None
    ).count()
