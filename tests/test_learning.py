...
def test_add_command(temp_db_path):
    ...
    conn = sqlite3.connect(temp_db_path)
    cursor = conn.cursor()
    cursor.execute("SELECT natural_text, generated_command, executed, success FROM commands WHERE id = ?", (command_id,))
    row = cursor.fetchone()
    conn.close()
...
def test_add_feedback(temp_db_path):
    ...
    conn = sqlite3.connect(temp_db_path)
    cursor = conn.cursor()
    cursor.execute("SELECT feedback_type, feedback_text FROM feedback WHERE command_id = ?", (command_id,))
    row = cursor.fetchone()
    conn.close()
