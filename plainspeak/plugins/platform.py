...
class PlatformManager:
    ...
    def is_safe_path(self, path: Union[str, Path]) -> bool:
        ...
        safe_roots = [
            self.get_known_path('temp'),
            self.get_known_path('downloads')
        ]
        
        if self.is_windows:
            safe_roots.extend([
                self.get_known_path('program_files'),
                self.get_known_path('system')
            ])
        elif self.is_macos:
            safe_roots.extend([
                self.get_known_path('applications'),
                self.get_known_path('library')
            ])
        else:
            safe_roots.extend([
                self.get_known_path('bin'),
                self.get_known_path('local')
            ])
