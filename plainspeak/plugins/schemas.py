...
class CommandConfig(BaseModel):
    ...
    template: str = Field(
        ...,
        description="Jinja2 template for generating the command"
    )
...
class EntryPointConfig(BaseModel):
    ...
    class_path: str = Field(
        ...,
        description="Import path to the plugin class"
    )
...
class PluginConfig(BaseModel):
    ...
    instance: Optional[Any] = Field(
        None,
        description="Instance of the plugin class once loaded"
    )
    enabled: bool = Field(
        True,
    ...
    load_error: Optional[str] = Field(
        None,
        description="Error message if plugin failed to load"
    )
