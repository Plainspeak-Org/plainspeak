...
ALLOWED_COMMANDS = {
    SecurityLevel.LOW: [
        ...
        "EXPLAIN", "ANALYZE", "WITH", "PRAGMA", "SHOW"
    ],
    SecurityLevel.MEDIUM: [
        "SELECT", "INSERT", "UPDATE", "CREATE TABLE", "CREATE VIEW",
        "ALTER TABLE", "WITH", "EXPLAIN", "ANALYZE", "PRAGMA", "SHOW"
    ],
    SecurityLevel.HIGH: [
        "SELECT", "EXPLAIN", "ANALYZE", "PRAGMA", "SHOW"
    ],
...
DANGEROUS_PATTERNS = [
    (r";\s*[^\s]", "Multiple statements detected"),  # SQL injection via multiple statements
    (r"--", "SQL comment detected"),                  # Comments might hide malicious code
    (r"/\*.*?\*/", "SQL comment block detected"),     # Block comments might hide malicious code
...
class SQLSecurityChecker:
    ...
    def validate_query(self, query: str) -> Tuple[bool, Optional[str]]:
        ...
        if command not in allowed:
            return False, f"Command '{command}' not allowed at security level {self.security_level.name}"
